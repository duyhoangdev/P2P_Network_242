...
def announce(announce: schemas.AnnounceRequest = Depends(), db: Session = Depends(database.get_db),
             current_user: models.User = Depends(auth.get_current_user)):
    file = crud.get_file_by_info_hash(db, announce.info_hash)
    if not file:
        raise HTTPException(status_code=404, detail="File not found")
    peer = crud.create_or_update_peer(db, current_user.id, announce.peer_id, announce.ip, announce.port)
    crud.update_peer_file(db, peer.id, file.id, announce)
    peers = crud.get_peers_for_file(db, file.id)
    return {"peers": peers}
@app.get("/test")
def test():
